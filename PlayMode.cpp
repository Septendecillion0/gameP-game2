...
bool GameOver = false;
bool GameStart = true;
float PeeTimer = 5.0f; //must be global so we can draw later
...
void PlayMode::update(float elapsed) {
	...
	//update PeeTimer
	static float StartTimer = 5.0f;
	static float PeeTimerMax = 5.0f;
	...
	if (glm::all(glm::lessThanEqual(glm::abs(pos), glm::vec3(10.0f)))) {
		...
		glm::vec3 new_pos;
		while (glm::all(glm::lessThanEqual(glm::abs(new_pos), glm::vec3(15.0f)))) {
			new_pos = glm::vec3(
				randf(-50.0f, 50.0f),
				randf(-50.0f, 50.0f),
				7.0f
